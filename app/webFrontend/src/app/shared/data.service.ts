...
export class DataService {
    ...
    constructor(public apiPath: string, public backendService: BackendService, public changeProps2Date?: string[], public dependentID?: string) {}
    ...
    deleteItem(deleteItem: any): Promise<any> {
        return new Promise((resolve, reject) => {
            this.backendService.delete(this.apiPath + deleteItem._id)
                .subscribe(
                    () => {
                        this.items = this.items.filter(item => {return item._id !== deleteItem._id});
                        resolve()
                    },
                    error => reject(error)
                )
